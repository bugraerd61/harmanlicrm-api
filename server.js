...
async function initDB() {
  await pool.query('CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY,email VARCHAR(255) UNIQUE NOT NULL,password VARCHAR(255) NOT NULL,name VARCHAR(255),created_at TIMESTAMP DEFAULT NOW())');
  await pool.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS active BOOLEAN DEFAULT FALSE');
  await pool.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS role VARCHAR(50) DEFAULT \'user\'');
  await pool.query('CREATE TABLE IF NOT EXISTS excel_uploads (id SERIAL PRIMARY KEY,etiket VARCHAR(255),tarih TIMESTAMP DEFAULT NOW(),uploaded_by INTEGER REFERENCES users(id),fat_data JSONB,akt_data JSONB,sat_data JSONB,tek_data JSONB,meta JSONB)');
  await pool.query('CREATE TABLE IF NOT EXISTS kv_store (key VARCHAR(255) PRIMARY KEY,value TEXT,updated_at TIMESTAMP DEFAULT NOW())');
  await pool.query('UPDATE users SET active=TRUE, role=\'admin\' WHERE email=$1', [ADMIN_EMAIL]);
...
app.post('/api/register', function(req, res) {
  ...
  bcrypt.hash(password, 10).then(function(hash) {
    return pool.query('INSERT INTO users (email, password, name, active, role) VALUES ($1, $2, $3, $4, $5) RETURNING id, email, name, role, active',
  ...
  }).catch(function(err) { res.status(400).json({ error: err.message }); });
});
...
app.post('/api/uploads', auth, function(req, res) {
  var { etiket, fat=[], akt=[], sat=[], tek=[], meta={} } = req.body;
  pool.query('INSERT INTO excel_uploads (etiket, uploaded_by, fat_data, akt_data, sat_data, tek_data, meta) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id, etiket, tarih, meta',
...
app.get('/api/uploads/:id', auth, function(req, res) {
  pool.query('SELECT * FROM excel_uploads WHERE id=$1', [req.params.id]).then(function(result) {
    ...
    res.json({ id: row.id, etiket: row.etiket, tarih: row.tarih, meta: row.meta,
      data: { fat: row.fat_data||[], akt: row.akt_data||[], sat: row.sat_data||[], tek: row.tek_data||[] } });
  }).catch(function(err) { res.status(500).json({ error: err.message }); });
});
...
app.post('/api/kv/:key', auth, function(req, res) {
  ...
  pool.query('INSERT INTO kv_store (key, value, updated_at) VALUES ($1,$2,NOW()) ON CONFLICT (key) DO UPDATE SET value=$2, updated_at=NOW()', [key, value])
    .then(function() { res.json({ ok: true, key }); })
    .catch(function(err) { res.status(500).json({ error: err.message }); });
});
...
app.post('/api/ai', auth, function(req, res) {
  ...
  var body = JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 500, messages: [{ role: 'user', content: prompt }] });
  var options = {
  ...
  var reqAI = https.request(options, function(resAI) {
    ...
    resAI.on('end', function() { try { res.json(JSON.parse(data)); } catch(e) { res.status(500).json({ error: 'Parse hatasi' }); } });
  });
  reqAI.on('error', function(e) { res.status(500).json({ error: e.message }); });
  reqAI.write(body); reqAI.end();
});
...
app.listen(PORT, function() {
  ...
  }).catch(function(err) { console.error('DB hatasi:', err.message); process.exit(1); });
});
